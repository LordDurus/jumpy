		...
		match section {
			...
			Section::Entities => {
				if line.starts_with("player_start") {
					...
					current_entity_kind = Some(EntityKindSource::Pickup { pickup_kind, value: 0 });
					current_entity_x = 0;
					current_entity_y = 0;
					section = Section::EntityBody;
				} else {
			...
			Section::EntityBody => {
				if line.starts_with("x") {
					...
					match current_entity_kind {
						Some(EntityKindSource::Enemy { ref mut patrol_min, .. }) => {
							*patrol_min = value;
						}
						_ => {
					...
					match current_entity_kind {
						Some(EntityKindSource::Enemy { ref mut patrol_max, .. }) => {
							*patrol_max = value;
						}
						_ => {
					...
					match current_entity_kind {
						Some(EntityKindSource::Pickup { value: ref mut v, .. }) => {
							*v = value;
						}
						_ => {
			...
			Section::TriggerBody => {
				if line.starts_with("x") {
					...
					match current_trigger_kind {
						Some(TriggerKindSource::Message { ref mut text_id, .. }) => {
							*text_id = s;
						}
						_ => {
...
fn parse_header_line(line: &str, existing: Option<LevelHeader>) -> Result<LevelHeader, String> {
	...
	match key {
		...
		"tile_size" => {
			header.tile_size = value_str.parse::<u32>().map_err(|e| format!("invalid tile_size value '{}': {}", value_str, e))?;
		}
		"gravity" => {
