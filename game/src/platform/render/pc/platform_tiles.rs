...
impl PcRenderer {
	pub fn draw_platform_entity_tiles(
		...
		let dest_width_pixels: u32 = (tile_width_world * scale) as u32;
		let dest_height_pixels: u32 = (tile_height_world * scale) as u32;
		...
		for i in 0..width_tiles {
			...
			let screen_left: i32 = ((seg_world_left - camera_left) * scale) as i32;
			let screen_top: i32 = ((seg_world_top - camera_top) * scale) as i32;

			let src = Rect::new(source_left, source_top, tile_pixel, tile_pixel);
			let dst = Rect::new(screen_left, screen_top, dest_width_pixels, dest_height_pixels);
